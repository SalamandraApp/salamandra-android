package com.android.salamandra._core.data.timeFormatter
...
class TimeFormatterImpl @Inject constructor() : TimeFormatter {
    ...
    override fun toTimeInterval(totalSeconds: Int, format: TimeIntervalFormat): String {
    ...
    override fun toTimeInterval(totalMilliseconds: Long, format: TimeIntervalFormat): String {
        val totalSeconds = (totalMilliseconds / 1000).toInt()

        return when (format) {
            TimeIntervalFormat.HHcMMcSS -> toTimeInterval(
                totalSeconds,
                format
            )
            ...
            TimeIntervalFormat.MMaSSa -> toTimeInterval(
                totalSeconds,
                format
            )
            TimeIntervalFormat.SSa -> toTimeInterval(
                totalSeconds,
                format
            )
        }
